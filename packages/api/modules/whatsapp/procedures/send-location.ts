...
export const sendLocation = protectedProcedure
	.route({
		method: "POST",
		path: "/whatsapp/send-location",
		tags: ["WhatsApp"],
		summary: "Send a location",
		description: "Sends a location pin to a chat and records it outbound.",
	})
	.input(
		z.object({
			chatId: z.string(),
	...
	.handler(async ({ input, context: { user, session } }) => {
		...
		try {
			result = await openwa.sendLocation(sender.openwaSessionId, {
				chatId: input.chatId,
			...
			logger.error(error, { ctx: "whatsapp.sendLocation", chatId: input.chatId });
			throw new ORPCError("INTERNAL_SERVER_ERROR");
		}

		const label = input.description ?? input.address;
		const body = label
			? `[location] ${label} (${input.latitude}, ${input.longitude})`
